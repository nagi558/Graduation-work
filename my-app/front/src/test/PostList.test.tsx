...
describe("PostList", () => {
  ...
  describe("自分の投稿タブ", () => {
    ...
    it("検索結果が0件の場合メッセージが表示される", async () => {
      ...
      await screen.findByText("テストタイトル")
      await user.type(
        screen.getByPlaceholderText("タイトル検索"),
        "存在しないタイトル",
      )
      await user.click(screen.getByRole("button", { name: "検索" }))
  ...
  describe("Pair機能", () => {
    it("Pair未接続の場合タブが表示されない", async () => {
      ...
      expect(
        screen.queryByRole("button", { name: "パートナーの投稿" }),
      ).not.toBeInTheDocument()
    })
    ...
    it("Pair接続済みの場合タブが表示される", async () => {
      ...
      renderPostList()
      expect(
        await screen.findByRole("button", { name: "自分の投稿" }),
      ).toBeInTheDocument()
      expect(
        await screen.findByRole("button", { name: "パートナーの投稿" }),
      ).toBeInTheDocument()
    })
    ...
    it("パートナーの投稿タブには編集・削除ボタンが表示されない", async () => {
      ...
      expect(
        screen.queryByRole("button", { name: "修正" }),
      ).not.toBeInTheDocument()
      expect(
  ...
  describe("Guide,Info,Toast", () => {
    ...
    it("初回ユーザーはGuideModalが表示される", async () => {
      vi.mocked(axiosInstance.get).mockImplementation((url) => {
        if (url === "/api/v1/user") {
          return Promise.resolve({ data: { has_seen_guide: false } } as any)
        }
        if (url === "/api/v1/posts") {
          return Promise.resolve({ data: mockPosts } as any)
        }
    ...
    it("GuideModalを閉じるとupdate_guide APIが呼ばれる", async () => {
      vi.mocked(axiosInstance.get).mockImplementation((url) => {
        if (url === "/api/v1/user") {
          return Promise.resolve({ data: { has_seen_guide: false } } as any)
        }
      ...
      vi.mocked(axiosInstance.patch).mockResolvedValue({} as any)

      const user = userEvent.setup()
      renderPostList()

      const dialogs = await screen.findAllByRole("dialog")
      await user.click(within(dialogs[0]).getByText("スキップ"))

      await waitFor(() => {
        expect(axiosInstance.patch).toHaveBeenCalledWith(
          "/api/v1/user/update_guide",
        )
      })
    })
    ...
    it("Guide済み & 投稿0件のときSimpleInfoModalが表示される", async () => {
      ...
      expect(
        await screen.findByText("「新規作成」から書いてみましょう"),
      ).toBeInTheDocument()
    })
    ...
    it("SimpleInfoModalの閉じるでModalが消える", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.queryByText("「新規作成」から書いてみましょう"),
        ).not.toBeInTheDocument()
      })
    })
    ...
    it("Toastは閉じるボタンで消える", async () => {
      ...
      const toastClose = await screen.findByTestId("toast-close")
      await user.click(toastClose)

      await waitFor(() => {
        expect(
          screen.queryByText("記録を残しました。未来へのメッセージになります"),
        ).not.toBeInTheDocument()
      })
    })
  })
